...
#if (JEFFV != u1g_EJCC_NOT_USE)  && ((JEEFI == u1g_EJCC_DUAL)  || (JEEFI == u1g_EJCC_D4))  && (JEEGMG_E == u1g_EJCC_HVPLGR_E)  && (JEMDVVT == u1g_EJCC_USE)   /* Alcohol fuel combustion improvement ID */
#define u1g_EMINJ_CIMPALC_ID               ((u1)(46))         /* アルコール燃料燃焼改善制御(ID) */
...
typedef struct
{
    ...
    s4 s4_eqinjplfix[8];              /* lsb=(4096*1024)/128/256/256/256,unit=mg/st :PL固定噴射量 */
    u1 u1_stepkcyl;                   /* lsb=1 :気筒別噴射量補正段 */
    f4 f4_kqcyl[u1g_EJCC_NCYL];       /* unit=倍 :気筒別噴射量補正係数 */
...
#define u4g_EMINJ_INJPTN_VD ((u4)(0x00000000U))                         /* m=eminj,lsb=,ofs=,unit= :噴射ﾊﾟﾀｰﾝ初期値 */

#else                                                                   /* 【D-4以外】 */
#define u4g_EMINJ_INJPTN_VD ((u4)(0x00000000U))                         /* m=eminj,lsb=,ofs=,unit= :噴射ﾊﾟﾀｰﾝ初期値 */
#endif                                                                  /* JEEFI */

/* Cylinder-specific injection correction constants (acw-16-a-t new) */
#define u1g_EMINJ_STEPKCYL_VD                    0b11111111           /* 気筒別噴射量補正段の初期値 */
#define s2g_EMINJ_KQCYL_VD                       1                    /* 気筒別噴射量補正係数の初期値 */
#define s2s_EMINJ_KQCYL_MN                       0                    /* 気筒別噴射量補正係数の最小値 */
#define s2s_EMINJ_KQCYL_MX                       2                    /* 気筒別噴射量補正係数の最大値 */
#define u4g_EMINJ_RQSTEPKCYL                     0x00004000           /* 気筒別噴射量補正段の要求フラグ */
#define u4g_EMINJ_RQKQCYL                        0x00008000           /* 気筒別噴射量補正係数の要求フラグ */
#define u1g_EMINJ_STEPKCYL1                      0b10000000           /* 気筒別噴射量補正段（第1段） */
#define u1g_EMINJ_STEPKCYL2                      0b01000000           /* 気筒別噴射量補正段（第2段） */
#define u1g_EMINJ_STEPKCYL3                      0b00100000           /* 気筒別噴射量補正段（第3段） */
#define u1g_EMINJ_STEPKCYL4                      0b00010000           /* 気筒別噴射量補正段（第4段） */
#define u1g_EMINJ_STEPKCYL5                      0b00001000           /* 気筒別噴射量補正段（第5段） */
#define u1g_EMINJ_STEPKCYL6                      0b00000100           /* 気筒別噴射量補正段（第6段） */
#define u1g_EMINJ_STEPKCYL7                      0b00000010           /* 気筒別噴射量補正段（第7段） */
#define u1g_EMINJ_STEPKCYL8                      0b00000001           /* 気筒別噴射量補正段（第8段） */
...
#if JEGORO_E == u1g_EJCC_USE   /* GORO noise countermeasure ID */
#define u1g_EMINJ_GORO_ID                 ((u1)(100))         /* ゴロゴロ音対策実現部要求(ID) */
#endif                                                                  /* JEGORO_E */
