...
#if (JEEFI == u1g_EJCC_DUAL) || (JEEFI == u1g_EJCC_D4)                  /* 【ﾃﾞｭｱﾙINJ】OR【D-4】 */
volatile const u1 u1g_eminj_STRATIST_PRI = (u1)(((40.)/(1.))+0.5);      /* m=eminj,lsb=1,ofs=,unit=- :成層始動制御の優先度 */
#endif                                                                  /* JEEFI */

#if JEEFI == u1g_EJCC_DUAL                                              /* 【ﾃﾞｭｱﾙINJ】 */
volatile const u1 u1g_eminj_STAFRG_PRI = (u1)(((42.)/(1.))+0.5);        /* m=eminj,lsb=1,ofs=,unit=- :極低温始動制御の優先度 */
...
volatile const u1 u1g_eminj_CIMPALC_PRI = (u1)(((46.)/(1.))+0.5);        /* m=eminj,lsb=1,ofs=,unit=- :ｱﾙｺｰﾙ燃料燃焼改善制御要求時の優先度 */
#endif                                                                  /* CSW for CIMPALC */

#if (JEFFV != u1g_EJCC_NOT_USE)  && ((JEEFI == u1g_EJCC_DUAL)  || (JEEFI == u1g_EJCC_D4))  && (JEEGMG_E == u1g_EJCC_HVPLGR_E)  && (JEMDVVT == u1g_EJCC_USE)   /* CSW for CIMPALC */
volatile const u1 u1g_eminj_CIMPALC_PRI = (u1)(((46.)/(1.))+0.5);        /* m=eminj,lsb=1,ofs=,unit=- :ｱﾙｺｰﾙ燃料燃焼改善制御要求時の優先度 */
#endif                                                                  /* CSW */
...
volatile const u1 u1g_eminj_STAHV_PRI = (u1)(((44.)/(1.))+0.5);         /* m=eminj,lsb=1,ofs=,unit=- :HV始動制御の優先度 */
...
volatile const u1 u1g_eminj_GORO_PRI = (u1)(((100.)/(1.))+0.5);        /* m=eminj,lsb=1,ofs=,unit=- :ｺﾞﾛｺﾞﾛ音対策実現部要求の優先度 */
#endif                                                                  /* JEGORO_E */
