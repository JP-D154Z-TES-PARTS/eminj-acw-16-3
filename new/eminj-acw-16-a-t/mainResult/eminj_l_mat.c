...
#endif /* JEEFI */
#if JEEFI == u1g_EJCC_PORT      /*【ALL HV】AND【ﾎﾟｰﾄ】*/
...
#if ((JEALLHV_E == u1g_EJCC_ALLHV_E) && (JEEFI == u1g_EJCC_DUAL)) || (JEEFI == u1g_EJCC_PORT)       /*【ALL HV】AND【D-4以外】*/
...
#endif /* JEALLHV_E,JEPLMLT_E,EMINJ_BINJPLCTR_WC_MEDI */
#if JEEFI == u1g_EJCC_DUAL      /*【仕様INJ】*/
 #if JEFFV != u1g_EJCC_NOT_USE  /*【FFV制御有】*/
        { &vds_eminj_dummy_emedi_dataget2,   u1g_EMINJ_CIMPALC_ID   },    /* 優先度=46 :ｴﾀﾉｰﾙ燃料ｺﾝﾎﾟ噴射濃化要求 */
 #endif /* JEFFV */
#endif /* JEEFI */
#if (JEEGMG_E == u1g_EJCC_HVPLGR_E) && (JEFFV == u1g_EJCC_NOT_USE) && (JEPLMLT_E == u1g_EJCC_USE) && (EMINJ_BINJPLCTR_STAHV_MEDI == ON) /*【EG-MG遊星ｷﾞｱ】AND【FFV制御無】AND【PLﾏﾙﾁ噴射有】AND【ﾊﾟｰｼｬﾙﾘﾌﾄ噴射を用いた基本噴射制御(HV始動制御時)要求使用】*/
...
#pragma ghs startdata
                                                                          ...
                                                                          /*         :rap関数を作成し設定すること                                 */
    {
        ...
        { &vds_eminj_dummy_emedi_dataget2,  u1g_EMINJ_GORO_ID    },     /* 優先度=100 :ｺﾞﾛｺﾞﾛ音対策制御要求 */
